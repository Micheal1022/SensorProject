    ...
    if (size == 8) {
        if (m_recvByteArray.at(0) == m_hostNumber) {
            ...
            case 0x03:
                if (regAddr == 0x34) {
                    //mainpower
                    sendData(0x03,0x02334,m_mainPower);
                } else if(regAddr == 0x35) {
                    //backPower
                    sendData(0x03,0x02335,m_backPower);
                } else {
                    quint16 startaddr = (uchar)m_recvByteArray.at(2)<<8 | (uchar)m_recvByteArray.at(3);
                    int regCount = (uchar)m_recvByteArray.at(4) << 8 | (uchar)m_recvByteArray.at(5);
                    //第一通道
                    if (startaddr <= STARTADDR + NODENUM) {
                        ...
                        //第二通道
                        QByteArray dateArray;
                        startaddr = startaddr - STARTADDR - NODENUM;
                        for (int i = startaddr; i < startaddr + regCount;i++) {
                            dateArray.append(null_d);
                            dateArray.append(m_nodeArray[i].state);
                        }
...
void RS485::slotNodeState(QList<int> dataList)
{
    ...
    int pass  = dataList.at(1);
    int canId = dataList.at(2);
    int state = dataList.at(4);
    ...
    if (pass == PASS_1) {
        m_nodeArray[canId - 1].state = state;
    } else if (pass == PASS_2) {
        m_nodeArray[canId - 1+1024].state = state;
    }
}
